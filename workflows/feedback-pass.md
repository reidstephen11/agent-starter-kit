...
1. **Am I allowed to do this?** Identifiable student work through an AI tool is an
   ...
   whoever approves it at your school before the first pass, not after.
2. **Local agent only.** Runs from the locally-synced school folder, on your machine.
   No cloud/web agent mode, no personal account, no upload, no external service.
3. **Folder IDs, never names**, in every report, profile, prompt and filename.
4. **Mode.** Advisory (agent drafts, teacher enters) or writing (agent writes into the
   designated fields of student files). **Start in advisory, stay there for weeks.**
   Writing mode requires everything in "If you turn on writing mode" below.
...
- `_reports/students/<ID>.md` — a running per-student profile: agent-maintained patterns
  over time, and a section of **my** notes that you **read but never modify**.
